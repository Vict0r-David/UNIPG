...
def treeGrad(dico_Arg,dico_Att):
    dico_lvl_arg = {}
    for arg in dico_Arg:
        dico_lvl_arg[arg] = [0,set()]
    
    for att,l in dico_Att.items():
        dico_lvl_arg[l[1]][1].add(l[0])
    
    for arg,l in dico_lvl_arg.items():
        if l[1] != []:
            l[0] = max([ x + 1 for x in l[1]])
    return dico_lvl_arg
...
def grad(dico_Arg,dico_Att):
    ...
    for id_a,w_ar in dico_Arg.items():
        ...
        for id_att,att in dico_Att.items():
            if id_a == att[1]:
               deg = deg * (1-(dico_Arg[att[0]]*(-att[2])))
        new_arg[id_a] = deg
    return new_arg
